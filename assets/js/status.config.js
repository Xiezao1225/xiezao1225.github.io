 ...
 * ----------------------------------------------------------------------------
 * 这个文件是"唯一真相来源"。改完存盘、commit、push，两个页面同时生效。
 *
 * 页面分工（公开页和后台已经拆开了）：
 *   status.html  —— 公开页，别人看的。只有"读"，没有任何写入路径。
 *   admin.html   —— 后台，你自己用的。登录、改状态、实时预览、保存。
 *
 * provider 三选一：
 *   'local'    —— 不接后端。状态就写在本文件的 `fallback` 区块里，页面无法写入云端。
 *   'supabase' —— 推荐。任何人可读；只有你在 admin.html 里用邮箱+密码登录后才能写。
 *                 支持 Realtime 推送：你手机改完，别人开着的页面自动变。
 *   'jsonbin'  —— 备选。用 JSONBin 的 Master Key 写入，靠轮询同步。
 *
 * 后端的注册 / 建表 / 拿 key 步骤：见 admin.html 登录卡片下方那张"还没配好后端"的说明卡
 * （只有 provider 没配全时才会显示）。
 * ==========================================================================*/
